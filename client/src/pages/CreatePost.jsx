...
const CreatePost = () => {
  ...
  const handleSubmit = async () => {
    if (!content && images.length === 0 && videos.length === 0) {
      setAlert({ message: "Please add content, images, or videos", type: "warning" });
      return;
    }
    ...
    setTimeout(() => {
      ...
      setImages([]);
      setVideos([]);
      setAlert({ message: "Post published successfully!", type: "success" });
    }, 2000);
  };
      ...
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='max-w-xl bg-white p-4 sm:pb-3 rounded-xl shadow-md space-y-4'>
          ...
          <div className='flex items-center gap-3'>
            <img src={user.profile_image} alt="Profile" className='w-12 h-12 rounded-full shadow'/>
            <div>
            ...
            <div className='flex flex-wrap gap-2 mt-4'>
              {images.map((image, i) => (
                <div key={i} className='relative group'>
                  <img src={URL.createObjectURL(image)} alt="Upload" className="h-20 rounded-md"/>
                  <div
          ...
          {videos.length > 0 && (
            <div className='flex flex-wrap gap-2 mt-4'>
              {videos.map((video, i) => (
                <div key={i} className='relative group'>
                  <video src={URL.createObjectURL(video)} className="h-20 rounded-md" controls />
                  <div
